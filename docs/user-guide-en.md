...
- **Frequency** — word list with counts, per-1,000 rates, Gries **DP dispersion**, function-word filter, bar chart.
- **KWIC** — concordancer with configurable context window, case toggle, timestamps; exportable.
- **Keywords** — log-likelihood **G²** and **LogRatio** effect size (Hardie 2014) against your other sessions, or against a built-in function-word reference when only one session exists.
- **Collocations & N-grams** — bigram association (**MI**, **t-score**, **logDice**) plus top bigram/trigram/4-gram lists.
...
## 7. Local AI assistant and tools (optional)
