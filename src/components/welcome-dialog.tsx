...
export function WelcomeDialog({ lang, d, onStart }: { lang: Lang; d: Dict; onStart: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && close()}>
      <DialogContent className="max-w-lg gap-0 overflow-hidden rounded-2xl p-0 brand-gradient-soft" dir={ar ? "rtl" : "ltr"}>
        <DialogTitle className="sr-only">{t.title}</DialogTitle>
        <div className="flex items-center justify-center gap-3 pt-7">
          {pages.map((_, i) => (
            <span
              key={i}
              className={`h-1.5 rounded-full transition-all ${i === page ? "w-7 bg-cyan-500" : "w-3 bg-muted-foreground/30"}`}
              aria-hidden
            />
          ))}
        </div>
        ...
        <div className="grid gap-4 px-7 py-5 text-center">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-400/40 bg-cyan-400/10 glow-cyan">
            <PageIcon className="h-7 w-7 text-cyan-500 dark:text-cyan-300" />
          </div>
          <h2 className={`text-lg font-bold ${ar ? "font-arabic" : ""}`}>{pages[page].title}</h2>
          <p className={`text-sm leading-relaxed text-muted-foreground ${ar ? "font-arabic" : ""}`}>
            {pages[page].body}
          </p>
          ...
          {page === 2 && (
            <p className={`mx-auto flex max-w-sm items-start gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-2.5 text-start text-xs text-muted-foreground ${ar ? "font-arabic" : ""}`}>
        ...
        <div className="flex items-center justify-between border-t border-border/60 px-5 py-3.5">
          <Button variant="ghost" size="sm" onClick={close} className={ar ? "font-arabic" : ""}>
            {t.skip}
          </Button>
          <div className="flex items-center gap-2">
            {page > 0 && (
              <Button variant="outline" size="sm" onClick={() => setPage((p) => p - 1)} className={`gap-1.5 ${ar ? "font-arabic" : ""}`}>
            ...
            {isLast ? (
              ...
              <Button size="sm" onClick={() => setPage((p) => p + 1)} className={`gap-1.5 ${ar ? "font-arabic" : ""}`}>
                {t.next}
